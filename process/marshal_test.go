...
func TestMarshal(t *testing.T) {
			...
			},
			{
				Type: &Process_Node_Filter_{&Process_Node_Filter{
					...
					Conditions: []Process_Node_Filter_Condition{
						{Key: "x", Predicate: Process_Node_Filter_Condition_EQ, Value: "x"},
					},
				}},
			},
		},
	...
	var w2 Process
	assert.NoError(t, json.Unmarshal(val, &w2))
	...
	assert.Equal(t, w2.Nodes[4].GetFilter().Conditions[0].Predicate, Process_Node_Filter_Condition_EQ)
