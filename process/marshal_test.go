...
func TestMarshal(t *testing.T) {
	w := Process{
		...
		Graph: Graph{
			Nodes: []Node{
				...
				Filter{Key: "6", Filter: filter.Filter{
	...
	assert.Equal(t, len(w2.Nodes[4].(*Filter).Conditions), 1)
	assert.Equal(t, w2.Nodes[4].(*Filter).Conditions[0].Key, "x")
	assert.Equal(t, w2.Nodes[4].(*Filter).Conditions[0].Predicate, filter.EQ)
	assert.Equal(t, w2.Nodes[4].(*Filter).Conditions[0].Value, "x")
}
