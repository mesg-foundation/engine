...
func testOrchestratorRefGrandParentResult(executionStream pb.Execution_StreamClient, instanceHash, runnerHash hash.Hash) func(t *testing.T) {
	return func(t *testing.T) {
		t.Skip("not yet possible to ref a result")
		var processHash hash.Hash
		t.Run("create process", func(t *testing.T) {
			respProc, err := client.ProcessClient.Create(context.Background(), &pb.CreateProcessRequest{
				Name: "ref-grand-parent-result",
		...
		t.Run("trigger process", func(t *testing.T) {
			...
			t.Run("check trigger in progress execution", func(t *testing.T) {
			...
			t.Run("check trigger completed execution", func(t *testing.T) {
				exec, err := executionStream.Recv()
				require.NoError(t, err)
				require.Equal(t, "task2", exec.TaskKey)
