...
func initDatabases(cfg *config.Config) (*database.LevelDBInstanceDB, *database.LevelDBExecutionDB, *database.LevelDBWorkflowDB, error) {
	// init instance db.
	...
	return instanceDB, executionDB, workflowDB, nil
}
...
func main() {
	...
	// init databases
	instanceDB, executionDB, workflowDB, err := initDatabases(cfg)
	if err != nil {
