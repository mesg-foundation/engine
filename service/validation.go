...
func (s *Service) RequireEventData(eventKey string, eventData map[string]interface{}) error {
	...
	if len(validateParametersSchema(e.Data, eventData)) > 0 {
		return fmt.Errorf("InvalidEventDataError")
	}
	return nil
}
...
func (s *Service) RequireTaskInputs(taskKey string, taskInputs map[string]interface{}) error {
	...
	if len(validateParametersSchema(t.Inputs, taskInputs)) > 0 {
		return fmt.Errorf("InvalidTaskInputError")
	}
	return nil
}
...
func (s *Service) RequireTaskOutputs(taskKey string, taskOutputs map[string]interface{}) error {
	...
	if len(validateParametersSchema(t.Outputs, taskOutputs)) > 0 {
		return fmt.Errorf("InvalidTaskOutputError")
	}
	return nil
}
