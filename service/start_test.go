...
func TestExtractVolumes(t *testing.T) {
	...
	v, err := volumeInformations(s, "test", "foo")
	require.NoError(t, err)
	require.Equal(t, v.Source, volumes[0].Source)
	require.Equal(t, "foo", volumes[0].Target)
	require.Equal(t, false, volumes[0].Bind)
	v, err = volumeInformations(s, "test", "bar")
	require.NoError(t, err)
	require.Equal(t, v.Source, volumes[1].Source)
