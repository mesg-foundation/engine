...
var data = &Instance{
	ServiceHash: address.ServAddress(crypto.AddressHash([]byte("10"))),
	EnvHash:     hash.Int(1),
}
...
func TestHashSerialize(t *testing.T) {
	...
	require.Equal(t, "cosmos17zm8re7hr6m8e96n0vwf6guvffrl49z5td6gg0", address.InstAddress(crypto.AddressHash([]byte(data.HashSerialize()))).String())
}
