...
func newClient() *Client {
	return &Client{
		...
		requests: &requests{
			negotiateAPIVersion:   make(chan NegotiateAPIVersionRequest, 10),
			swarmInit:             make(chan SwarmInitRequest, 10),
			networkCreate:         make(chan NetworkCreateRequest, 10),
			info:                  make(chan InfoRequest, 10),
			containerList:         make(chan ContainerListRequest, 10),
			containerInspect:      make(chan ContainerInspectRequest, 10),
			containerStop:         make(chan ContainerStopRequest, 10),
			containerRemove:       make(chan ContainerRemoveRequest, 10),
			imageBuild:            make(chan ImageBuildRequest, 10),
			networkInspect:        make(chan NetworkInspectRequest, 10),
			networkRemove:         make(chan NetworkRemoveRequest, 10),
			taskList:              make(chan TaskListRequest, 10),
			serviceCreate:         make(chan ServiceCreateRequest, 10),
			serviceList:           make(chan ServiceListRequest, 10),
			serviceInspectWithRaw: make(chan ServiceInspectWithRawRequest, 10),
			serviceRemove:         make(chan ServiceRemoveRequest, 10),
			serviceLogs:           make(chan ServiceLogsRequest, 10),
			events:                make(chan EventsRequest, 10),
		},

		// buffered channels helps with writing tests in synchronous syntax while using
		// ProvideX methods of Testing.
		responses: &responses{
			info:                  make(chan infoResponse, 10),
			imageBuild:            make(chan imageBuildResponse, 10),
			networkInspect:        make(chan networkInspectResponse, 10),
			networkCreate:         make(chan networkCreateResponse, 10),
			taskList:              make(chan taskListResponse, 10),
			serviceCreate:         make(chan serviceCreateResponse, 10),
			serviceList:           make(chan serviceListResponse, 10),
			serviceInspectWithRaw: make(chan serviceInspectWithRawResponse, 10),
			serviceRemove:         make(chan serviceRemoveResponse, 10),
			serviceLogs:           make(chan serviceLogsResponse, 10),
			containerInspect:      make(chan containerInspectResponse, 10),
			containerList:         make(chan containerListResponse, 10),
			containerStop:         make(chan containerStopResponse, 10),
			containerRemove:       make(chan containerRemoveResponse, 10),
			events:                make(chan eventsResponse, 10),
		},
	}
}
