...
func (s *ProcessServer) Create(ctx context.Context, req *api.CreateProcessRequest) (*api.CreateProcessResponse, error) {
	...
	return &api.CreateProcessResponse{Hash: wf.Hash.String()}, nil
}
...
func (s *ProcessServer) Delete(ctx context.Context, request *api.DeleteProcessRequest) (*api.DeleteProcessResponse, error) {
	...
	return &api.DeleteProcessResponse{}, s.sdk.Process.Delete(hash)
}
...
func fromProtoProcessNodes(nodes []*types.Process_Node) ([]process.Node, error) {
	...
	for i, node := range nodes {
		...
		case *types.Process_Node_Event_:
			...
			res[i] = process.Event{Key: n.Event.Key, InstanceHash: hash, EventKey: n.Event.EventKey}
		case *types.Process_Node_Result_:
			hash, err := hash.Decode(n.Result.InstanceHash)
			if err != nil {
				return nil, err
			}
			res[i] = process.Result{Key: n.Result.Key, InstanceHash: hash, TaskKey: n.Result.TaskKey}
		case *types.Process_Node_Task_:
			hash, err := hash.Decode(n.Task.InstanceHash)
			if err != nil {
				return nil, err
			}
			res[i] = process.Task{InstanceHash: hash, TaskKey: n.Task.TaskKey, Key: n.Task.Key}
...
func toProtoProcesss(processes []*process.Process) []*types.Process {
