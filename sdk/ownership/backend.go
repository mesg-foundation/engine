...
func NewBackend(appFactory *cosmos.AppFactory) *Backend {
	...
	ownership.RegisterCodec(ModuleCdc)
	ownership.RegisterCodec(appFactory.Cdc())
	return backend
}

func (s *Backend) db(request cosmostypes.Request) *database.OwnershipDB {
	return database.NewOwnershipDB(store.NewCosmosStore(request.KVStore(s.storeKey)), ModuleCdc)
}
