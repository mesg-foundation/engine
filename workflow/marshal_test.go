...
func TestMarshal(t *testing.T) {
	w := Workflow{
		...
		Graph: Graph{
			Nodes: []Node{
				...
				Result{InstanceHash: hash.Int(2), TaskKey: "2"},
				Event{InstanceHash: hash.Int(3), EventKey: "3"},
				Mapping{Key: "4", Outputs: []Output{
	...
	assert.IsType(t, &Mapping{}, w2.Nodes[3])
	assert.Equal(t, w2.Nodes[3].(*Mapping).Key, "4")
	assert.Equal(t, len(w2.Nodes[3].(*Mapping).Outputs), 1)
	assert.Equal(t, w2.Nodes[3].(*Mapping).Outputs[0].Key, "5")
	assert.Equal(t, w2.Nodes[3].(*Mapping).Outputs[0].Ref.NodeKey, "5")
	assert.Equal(t, w2.Nodes[3].(*Mapping).Outputs[0].Ref.Key, "5")
}
