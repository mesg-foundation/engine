...
func createTestServcieWithInputs(inputs map[string]string) *definitions.Service {
	s := &definitions.Service{
	...
	for name, itype := range inputs {
		s.Tasks[0].Inputs = append(s.Tasks[0].Inputs, &definitions.Parameter{
			Key:      name,
