...
var (
	// DefaultCLIHome path for config and data generation.
	DefaultCLIHome  = os.ExpandEnv("$HOME/.mesg/app")
...
type App struct {
	*bam.BaseApp
	...
	serviceKeeper  serviceapp.Keeper
...
func New(logger log.Logger, db dbm.DB) (*App, *codec.Codec) {
	...
	var app = &App{
		...
		keyService:  sdk.NewKVStoreKey(serviceapp.StoreKey),
	}
	...
	app.serviceKeeper = serviceapp.NewKeeper(app.keyService, app.cdc)
	...
	app.mm = module.NewManager(
		...
		serviceapp.NewAppModule(app.serviceKeeper),
	)
...
func (app *App) ExportAppStateAndValidators() (json.RawMessage, []tmtypes.GenesisValidator, error) {
